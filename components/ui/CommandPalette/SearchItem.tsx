...
export default ({ item, ...props }: { item: Product; onClick: () => void }) => (
  <Link
    {...props}
    href={'/tool/' + item.slug}
    className="p-3 flex items-start gap-x-3 rounded-xl from-indigo-900/20 to-indigo-800/10 hover:bg-gradient-to-l border border-transparent hover:border-gray-800 duration-150">
    <ProductLogo src={item.logo_url} imgClassName="w-7 h-7" />

    <div className="text-sm">
      <ProductName className="text-gray-200">{item.name}</ProductName>
