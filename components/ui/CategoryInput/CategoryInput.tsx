...
export default ({ className, categories = [], setCategory = () => [] }: Props) => {
  ...
  return (
    <div className={`relative ${className}`}>
      <ul className="flex flex-wrap items-center gap-2 w-full px-3 py-2 border border-gray-800 rounded-lg">
        ...
        <li>
          <input
            ...
            className="text-sm text-gray-500 placeholder-gray-500 py-2 border-0 outline-none bg-transparent appearance-none"
            ...
            {value ? (
              newItems.length > 0 ? (
                newItems.map((item, idx) => (
                  <button
                    key={idx}
                    className="w-full text-left p-2 hover:bg-gray-700 active:bg-gray-600 rounded-lg duration-150"
                    onClick={() => setCategory([...categories, item])}>
                    {item.name}
                  </button>
                ))
              ) : (
                <div className="text-sm text-gray-300 p-2">No categories found</div>
              )
            ) : (
